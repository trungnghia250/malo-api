...
func NewSession(config *config.RabbitMQ) *Session {
	session := Session{
		logger: log.New(os.Stdout, "", log.LstdFlags),
		done:   make(chan bool),
		config: config,
	}
	go session.handleReconnect(config.AMQP)
	return &session
}
...
func (session *Session) handleReconnect(addr string) {
	for {
		...
		log.Println("Attempting to connect RabbitMQ")

		conn, err := session.connect(addr)

		if err != nil {
			log.Println("Failed to connect RabbitMQ. Retrying...")
...
func (session *Session) init(conn *amqp.Connection) error {
	...
	// todo: queue will be declared and bound to exchange in consumer service
	// declare queue
	...
	//for _, key := range strings.Split(session.config.RoutingKey, ",") {
	//	if err = ch.QueueBind(
	//		session.config.Queue,
	//		key,
	//		session.config.Exchange,
...
func (session *Session) Push(routingKey string, data []byte) error {
	if !session.isReady {
	...
	for {
		...
		case confirm := <-session.notifyConfirm:
			if confirm.Ack {
				//session.logger.Println("confirmed!")
				return nil
			}
		case <-time.After(resendDelay):
		}
